...
def read_hdr(path: str) -> np.ndarray:
    ...
    #将gamma矫正后的rgb保存为png
    cv2.imwrite('rgb_gamma.png', rgb)
    
    return rgb

def read_hdr_myVersion(path: str) -> np.ndarray:
    """Reads an HDR map from disk.
    ...
    #将hdr_image保存为png
    plt.imsave('hdr_image_raw.png', hdr_image)    
    ...
    #将gamma矫正后的hdr_image保存为png
    plt.imsave('hdr_image_gamma.png', hdr_image_corrected)
    #将hdr_image_corrected变为float32格式
    hdr_image_corrected = hdr_image_corrected.astype(np.float32)
     
    return hdr_image_corrected
...
def render_set(
    ...
    relight_path = os.path.join(model_path, name, f"ours_{iteration}_{random_suffix}", "relight")
    makedirs(relight_path, exist_ok=True)
    print(f"Rendering {name} set to {relight_path}")
...
def launch(
    ...
    hdri=read_hdr_myVersion(hdri_path)
    hdri = torch.from_numpy(hdri).cuda()
    res = 256
    cubemap = CubemapLight(base_res=res).cuda()
    cubemap.base.data = latlong_to_cubemap(hdri, [res, res])
    #将cubemap的6个方向的图像保存为png
    # for i in range(6):
    #     print("cubemap.base.data[i].shape: ", cubemap.base.data[i].shape)
    #     # Convert the tensor to a suitable format
    #     img = cubemap.base.data[i].cpu().float()
    #     img = transforms.ToPILImage()(img.permute(2, 0, 1))  # Ensure the tensor has 3 channels
    #     img.save(f'cubemap_{i}.png')

    #进一步修改cubemap的base.data,使得cubemap.base.data[0]有光照信息，其他5个方向的图像无光照信息
    # for i in range(1, 6):
    #     cubemap.base.data[i] = torch.zeros_like(cubemap.base.data[i])
    cubemap.eval()
