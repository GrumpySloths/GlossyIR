...
from myscene.cameras import  Camera
# from scene.dataset_readers import CameraInfo
# from train_consistency import CameraInfo
...
def loadCam_myversion(args:GroupParams, id: int, cam_info: CameraInfo) -> Camera:
    orig_w, orig_h = cam_info.image.size


    resolution=(int(orig_w),int(orig_h))
    resized_image_rgb = PILtoTorch(cam_info.gt_image, resolution)
    input_image=PILtoTorch(cam_info.input_image, resolution)
...
def cameraList_from_camInfos_myversion(
    cam_infos: List[CameraInfo], resolution_scale: float, args: GroupParams
) -> List[Camera]:
    camera_list = []

    for id, c in enumerate(tqdm(cam_infos)):
        camera_list.append(loadCam_myversion(args, id, c))
